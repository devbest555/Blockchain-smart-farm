...
const PotionsBar = artifacts.require('PotionsBar');

contract('PotionsBar', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.cake = await ElixirToken.new({ from: minter });
    this.syrup = await PotionsBar.new(this.cake.address, { from: minter });
  });
